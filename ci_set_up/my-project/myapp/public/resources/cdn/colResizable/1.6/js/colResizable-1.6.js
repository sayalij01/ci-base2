/**
	...
	v1.6.1	- Forked by Marco Eberhardt 
		- Use localStorage instead of sessionStorage
		- removed paddings from CSS rules
...
(function($){ 	
	...
	{
		console.log({"col-resize-table":tb, "col-resize-options":options});
        ...
        if(t.opt.disable){
        	return destroy(t);								// the user is asking to destroy a previously colResized table
        }
	...
	var destroy = function(t)
	{
		...
		if(!t || !t.is("table")) {
			return;	 									//if none, then it wasn't processed	 
		}
	...
	var createGrips = function(t){	
		...
		{											//iterate through the table column headers			
            ...
            {                        				 
                ...
                if(t.f) {
                	g.html("");                 	//if the table resizing mode is set to fixed, the last grip is removed since table with can not change
                }
            }
            
            g.bind('touchstart mousedown', onGripMouseDown); //bind the mousedown event to start dragging 
            
            if (!dc){	//if normal column bind the mousedown event to start dragging, if disabled then apply its css class 
	...
	{ 
		...
		{	//in serialization mode (after resizing a column)
			...
			if(!t.f) {
				S[t.id] += ";"+t.width();	//if not fixed, table width is stored
			}
		}	
	};
	...
	*/
	...
	{	
		...
		{	//if liveDrag is enabled
			...
			if (cb) { 
				e.currentTarget = t[0]; cb(e);		// if any, it is fired		 
			}					
		}
	...
	var onGripDragOver = function(e)
	{
        ...
        if (!(drag.x - drag.l == 0)) 
        {
            ...
            if (cb) { 
            	e.currentTarget = t[0]; cb(e);	// if there is a callback function, it is fired 
            }	
            
            if(t.p && S){
            	memento(t); 					// if postbackSafe is enabled and there is sessionStorage support, the new layout is serialized and stored
            }
        }
