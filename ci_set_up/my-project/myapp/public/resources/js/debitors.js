          ...
          function success(result) {
            if (result.error && result.error != "") {
              ...
              if(supervisor == 1 ){
                ...
                $('#btn_submit_supervisor_account').show();
                ...
                $('#btn_submit_supervisor_account').hide();
              }else{
...
$.debitors.show_hide_supervisor = function(){
    ...
    if(statisticGroup == '111' || statisticGroup == '112' || statisticGroup == '118') {
        ...
        // $('#fi_supervisor_id').css('display','none');
...
$.debitors.show_hide_debitor_as_delivery_block = function(){
  var debitor_as_delivery_account = $("#i_debitor_as_delivery_account").is(':checked');
  var i_delivery_account_id = $("#i_delivery_account_id").val();


    if(debitor_as_delivery_account  ){
      $("#i_hidden_delivery_account_id").val("");
      $("#i_delivery_account_id").val("");
      $('#i_hidden_delivery_account_id').attr('value', '');    
      $('#delivery_account_right').show();
      $('#delivery_account_left').hide();
      $('#i_use_own_address_for_delivery').prop('checked', false);
      $('#delivery_account_right select').attr("disabled", false);
      $('#delivery_account_right .checkbox-inline input').attr('disabled', false);
    }else if(i_delivery_account_id != ""){
      $('#delivery_account_data_debitor input').attr('readonly', true);
      $('#delivery_account_data_debitor select').attr("disabled", true);
      $('#delivery_account_data_debitor .checkbox-inline input').attr('disabled', true);
      // $('#delivery_account_right input').attr('readonly', true);
      // $('#delivery_account_right select').attr("disabled", true);
      // $('#delivery_account_right .checkbox-inline input').attr('disabled', true);
...
// alert(params.delivery_account_id)
  $.app.sendAjaxRequest(
...
$.debitors.set_debitor_contact_details = function(){
    var delivery_account_id = $("#i_delivery_account_id").val();
    var use_own_address_for_delivery = $("#i_use_own_address_for_delivery").is(':checked');
    var name_1 = $("#i_name_1").val();
    var name_2 = $("#i_name_2").val();
    var name_3 = $("#i_name_3").val();
    var zipcode = $("#i_zipcode").val();
    var debitor_location = $("#i_debitor_location").val();
    var street = $("#i_street").val();
    var country = $("#i_country").val();
    var subdivision = $("#i_subdivision").val();
    var district = $("#i_district").val();
    if(delivery_account_id != "" && use_own_address_for_delivery == 1){
      $("#i_contact_name1").val(name_1);
      $("#i_contact_name2").val(name_2);
      $("#i_contact_name3").val(name_3);
      $("#i_contact_zipcode").val(zipcode);
      $("#i_contact_ebitor_location").val(debitor_location);
      $("#i_contact_street").val(street);
      $("#i_contact_country").val(country).change();
      $("#i_contact_subdivision").val(subdivision).change();
      $("#i_contact_district").val(district);
    }
}
...
$.debitors.edit_debitor_contact = function (contact_id) {
  ...
  var on_content_replaced = function () {
   ...
   // $.debitors.set_debitor_contact_details();
 ...
 * save_supervisor
 */
$.debitors.create_supervisor_data = function (e) {
    e.preventDefault();
  var debitor_id = $("#i_debitor_id").val();
  var params = $("#full_form_supervisor").serializeArray();
  ...
  $.app.sendAjaxRequest(
    ...
    params,
   function success(result) {
      $.app.toConsole({ fkt: "callback save supervisor ajax", data: result });
      $.lang.item("supervisor_has_been_saved")
      $.app.redirect(
        baseUrl +"admin/debitors/edit/" +debitor_id
      );
    
    },
    true,
    null,
    $.lang.item("supervisor_data_save_progress")
  );
};
...
$.debitors.init_tab_customer = function () {
  if ($("#full_form_debitor").length > 0) {
    ...
    //get and set additional feilds
    $.debitors.get_debitor_delivery_id();
   /*  
    $("#i_supervisor_id").on("change", function () {
      var supervisor_id = $("#i_supervisor_id").val();
        if(supervisor_id != ""){
          $("#i_hidden_delivery_account_id").val("");
          $("#i_delivery_account_id").val("");
          $('#i_hidden_delivery_account_id').attr('value', '');    
          $("#i_delivery_account_id").val(supervisor_id);
          $("#i_hidden_delivery_account_id").val(supervisor_id);
          $.debitors.get_debitor_delivery_id();
 ...
 */
     ...
     $.debitors.show_hide_supervisor();
      $("#i_debitor_statistic_group").on("change", function () {
        $.debitors.show_hide_supervisor();
     });
...
$.debitors.init_tab_supervisor_account = function () {
      $("#full_form_supervisor").submit(function (e) {
      $.debitors.create_supervisor_data(e);
    });
...
$.debitors.init_tab = function (target) {
  switch (target) {
    ...
    case "#history":
      ...
      break;
    case "#betreuerkonto":
      $.debitors.init_tab_supervisor_account();
      break;
    default:
...
$.debitors.changeDeliveryAccount = function (id, account_number) {
  ...
  $('#i_debitor_as_delivery_account').attr('checked', false);
...
/*    var debitor_id = localStorage.getItem("delivery_debitor_id");
   $.debitors.set_debitor_delivery_data(debitor_id); */
    ...
    /*  $('#i_supervisor_id').change(function(){
      var supervisor_id = $("#i_supervisor_id").val();
        if(supervisor_id != ""){
          $("#i_hidden_delivery_account_id").val("");
          $("#i_delivery_account_id").val("");
      ...
      $.debitors.get_debitor_delivery_id();
        ...
        $('#i_supervisor_department').change(function(){
          var department_val = $("#i_supervisor_department option:selected").attr("label");
          if(department_val==50000)
          {
            $("#i_supervisor_costunit").val(241).change();
          }
          else if(department_val==58000)
          {
            $("#i_supervisor_costunit").val(576).change();
          }
          else
          {
      ...
      // $('#i_supervisor_department').change(function(){
      //   var department_val = $("#i_supervisor_department option:selected").attr("label");
      //   $("#i_supervisor_costunit option[data-costunit='" + department_val +"']").attr("selected","selected").change();
      // });

      // $('#i_supervisor_costunit').change(function(){
      //   var costunit_val = $("#i_supervisor_costunit option:selected").attr("data-costunit");
      //   $("#i_supervisor_department option[label='" + costunit_val +"']").attr("selected","selected").change();
      //   // alert($("#i_supervisor_department option[label]");
      //   // if($("#i_supervisor_department option[label]").val()==costunit_val)
      //   // {
      //   //   alert(hi);
      //   // }
      // });
  ...
  // $('#i_supervisor_costunit').change(function(){
  //         var costunit_val = $("#i_supervisor_costunit option:selected").attr("id");
  //         if(costunit_val==241)
  //         {
  //           $("#i_supervisor_department").val('DEPARTMENT_HOMECARE_PLUS').change();
  //         }
  //         else if(costunit_val==576)
  //         {
  //           $("#i_supervisor_department").val('DEPARTMENT_HOMECARE').change();
  //         }
  //         else
  //         {
  //           $("#i_supervisor_department").val('').change();
  //         }
  //       });
